...
class IntelPEDACmd(PEDACmd):
    ...
    def _eval_eval_target(self, inst):
        """
        ...
        if not m:
            ...
            if "]" in m.group(2): # e.g DWORD PTR [ebx+0xc]
                ...
                m = p.search(inst)
            target = self.peda.parse_and_eval("%s[%s]" % (m.group(1), m.group(2).strip()))
    ...
    def _testjump(self, inst=None):
        ...
        next_addr = self._eval_eval_target(inst)
    ...
    def context(self, *arg):
        ...
        try:
            #msg("arg:" + repr(*arg))
           #msg("config:" + repr(config.Option.get("context")))
           if self.pyInteractMode == True:
               super(IntelPEDACmd, self).context(*arg)
               #super(PEDACmd, self).context(["all"])
        except Exception as e:
            #msg("Exception" + repr(e))
            raise e

    def _diy_hexdump(self, arg):
        """
        ...
        if bytes_ is None:
            return ("Cannot retrieve memory content")
        else:
            linelen = 16  # display 16-bytes per line
            i = 0
            text = ""
    ...
    def __format_address(self, addr, type):
        """Colorize an address"""
        colorcodes = {
            "data": "blue",
            "code": "red",
    ...
    def __format_reference_chain(self, chain):
        """
        ...
        if not chain:
            ...
            for (v, t, vn) in chain:
                if t != "value":
                    l.append("%s%s " % ("--> " if not first else "", self.__format_address(v, t)))
                else:
    ...
    def _diy_telescope(self, address, count):
        """
        ...
        for chain in result:
            text += "%04d| " % (idx)
            text += self.__format_reference_chain(chain)
            text += "\n"
            idx += step
    ...
    def _get_args(self):
        ...
        args = self._get_function_args()

        back_info = "-"*40 + "\n"
        if args:
            back_info += "Guessed arguments:\n"
            for (i, a) in enumerate(args):
                chain = self.peda.examine_mem_reference(a)
                back_info += "arg[%d]: %s\n" % (i, format_reference_chain(chain))
        else:
            back_info += "No argument"

        return back_info


    def _beautiful_code(self, pc, count):
        """
        Display nearby disassembly at $PC of current execution context
        Usage:
            MYNAME [linecount]
            ...
            # stopped at jump
                ...
                if jumpto:  # JUMP is taken
                    ...
                    for line in code[1:]:
                        text += "       %s\n" % line.strip()
                    text += "-"*40 + "\n"
                    text += red("JUMP is taken")
                else:  # JUMP is NOT taken
                    text += format_disasm_code_intel(peda.disassemble_around(pc, count), pc)
                    
                    text += "\n"
                    text += "-"*40 + "\n"
                    text += green("JUMP is NOT taken")

                return text.rstrip()
            # stopped at other instructions
            else:
                text += self.peda.disassemble_around(pc, count)
                return (format_disasm_code_intel(text, pc))
        else:  # invalid $PC
            return ("Invalid $PC address: 0x%x" % pc)

    def _diy_get_code(self, arg):
        """
        not export
        Usage:
            MYNAME count
        ...
        if count is None:
            count = 8
            if address is not None and address < 0x1000:
                count = address
                address = None
        ...
        #msg(text)
        if inst:  # valid $PC
            text = self._beautiful_code(pc, count)
            #msg(format_disasm_code(text, pc))
            return text
        else:  # invalid $PC
            return "Invalid $PC address: 0x%x"%pc

    def _diy_get_stack(self, arg):
        """
        not export
        Usage:
            MYNAME count
        ...
        if args != []:
            ...
            if len(args) > 1:
                count = args[1]
        return self._diy_telescope(address, count)

    def _diy_get_regs(self, arg):
        """
        not export
        Usage:
            MYNAME count
        """
        #msg("in _diy_get_regs")
        return self.peda.getregs()

    def _diy_get_reg(self, arg):
        """
        not export
        Usage:
            MYNAME count
        ...
        args = normalize_argv(arg, 0)
        reg = args[0]
        return self.peda.getreg(reg)

    def _diy_set_reg(self, arg):
        """
        not export
        Usage:
            MYNAME count
        ...
        args = normalize_argv(arg, 0)
        reg, value = args
        return self.peda.execute_redirect("set $%s = 0x%x" % (reg, value))

    def _diy_stepi(self, arg):
        """
        not export
        Usage:
            MYNAME count
        ...
        if args != []:
            count = args[0]
        for i in range(count):
            self.peda.execute_redirect("si")

        return ""

    def _diy_stepover(self, arg):
        """
        not export
        Usage:
            MYNAME count
        ...
        for i in range(count):
            #self.peda.stepover()
            self.peda.execute_redirect("ni")
        return ""

    def _diy_continue(self, arg):
        """
        not export
        Usage:
            MYNAME count
    ...
    def _diy_read_mem(self, arg):
        ...
        #msg("args:" + repr(args))
        address, size = args
        data = self.peda.readmem(address, size)
        #data = str(data,'utf-8')
        #data = codecs.encode(data, "hex")
        #msg("type0:" + repr(type(data)))
        #msg("type1:" + repr(type(data)))
        #msg(str("data:") + data)
        #msg("type2:" + repr(type(data)))
        return data

    def _diy_write_mem(self, arg):
        """
        not export
        Usage:
            MYNAME count
        """
        #msg("in _diy_write_mem")
        address, data = list(arg)
        #msg("address:" + repr(address))
        address = to_int(address)
        #msg(data)
        #msg("type:" + repr(type(hex2str(data))))
        return self.peda.writemem(address, hex2str(data))

    def _diy_set_breakpoint(self, arg):
        """
        not export
        Usage:
            MYNAME count
        ...
        if len(args) > 1:
            ...
            if ttt == "temp":
                ...
                hard = True
        return self.peda.set_breakpoint_redirect(address, temp, hard)

    def _diy_get_breakpoint(self, arg):
        """
        not export
        Usage:
            MYNAME count
        ...
        args = normalize_argv(arg, 0)
        if len(args) > 0:
            return self.peda.get_breakpoint(args[0])

        return self.peda.get_breakpoints()

    def _diy_del_breakpoint(self, arg):
        """
        not export
        Usage:
            MYNAME count
    ...
    def _diy_set_interact_mode(self, arg):
        ...
        args = normalize_argv(arg, 0)
        if args[0] == 0:
            self.pyInteractMode = False
            enable_color(False)
            peda.execute("set prompt %s" % (""))  # custom prompt
        else:
            #msg("in context")
            self.pyInteractMode = True
            enable_color(True)
            #peda.execute("set prompt \001%s\002" % red("\002gdb-peda\001"))  # custom prompt
            peda.execute("set prompt %s" % red("gdb-peda $"))  # custom prompt
            config.Option.set("context", "all")

    def _diy_get_dbg_pid(self, arg):
        """
        not export
        Usage:
            MYNAME count
    ...
    function_map = {}
    function_map["get_code"] = _diy_get_code
    function_map["get_stack"] = _diy_get_stack
    function_map["get_regs"] = _diy_get_regs
    function_map["get_reg"] = _diy_get_reg
    function_map["set_reg"] = _diy_set_reg
    function_map["stepi"] = _diy_stepi
    function_map["stepover"] = _diy_stepover
    function_map["continue"] = _diy_continue
    function_map["read_mem"] = _diy_read_mem
    function_map["write_mem"] = _diy_write_mem
    function_map["write_mem"] = _diy_write_mem
    function_map["set_breakpoint"] = _diy_set_breakpoint
    function_map["del_breakpoint"] = _diy_del_breakpoint
    function_map["get_breakpoint"] = _diy_get_breakpoint
    function_map["gdb_cmd"] = _diy_gdb_cmd
    function_map["set_interact_mode"] = _diy_set_interact_mode
    function_map["get_dbg_pid"] = _diy_get_dbg_pid
    function_map["hexdump"] = _diy_hexdump

    def pyCmdRet(self, *arg):
        """
        Do pyCmd with python
        Usage:
            MYNAME pycmdline
        """
        args = normalize_argv(arg, 0)
        ...
        if cmdline in self.function_map.keys():
            ...
            try:
                ...
                #msg("type:" + repr(type(data)))
                #test = str2hex(data.encode('utf-8'))
                #data = to_hexstr(data)
                #data = self.function_map[cmdline](args[1:])
                #data = self.test_func(args[1:])
                code = 1
            except Exception as e:
            ...
            content = json.dumps({"data":data, "code":code})
            msg("pyCmd-B{%s}pyCmd-E"%str2hex(content.encode('utf-8')).decode('utf-8'))
        else:
            content = json.dumps({"data":"function not found", "code":-1})
            msg("pyCmd-B{%s}pyCmd-E"%str2hex(content.encode('utf-8')).decode('utf-8'))

    pyInteractMode = False
    def pyCmd(self, *arg):
        """
        Do pyCmd with python
        Usage:
        ...
        if cmdline in self.function_map.keys():
            ...
            try:
                #data = self._diy_gdb_cmd(args[1:])
                data = self.function_map[cmdline](self, args[1:])
                #msg("type:" + repr(type(data)))
                #data = to_hexstr(data)
                #data = self.function_map[cmdline](args[1:])
                #data = self.test_func(args[1:])
            ...
            if data is None:
                data = ''
            #msg("pyCmd-B{%s}pyCmd-E"%to_hexstr(data))
...
if __name__ == '__main__':
    ...
    for cmd in pedacmd.commands:
        ...
        if cmd not in ["help", "show", "set"]:# and cmd.startswith("_diy_") == False:
